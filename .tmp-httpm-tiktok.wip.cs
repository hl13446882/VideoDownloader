...
public sealed class HttpMediaDownloader
{
        ...
        CancellationToken ct)
    {
        ...
        // TikTok/Douyin signed progressive CDNs often advertise inflated Content-Length then RST.
        var softSignedProgressive = IsSoftSignedProgressiveHost(job.Variant.SourceUrl);
             ...
             written != response.Content.Headers.ContentRange!.To!.Value + 1))
        {
            ...
            if (!((softMedia && received >= 1024) ||
                  (softSignedProgressive && IsSoftSignedProgressiveAcceptable(received))))
                throw new DownloadException(ErrorCodes.IncompleteDownload, "Response body length does not match its range.");
            _logger.LogWarning(
                "Soft download length mismatch (got {Got}, declared {Declared}); accepting host={Host}",
                received,
                response.Content.Headers.ContentLength,
                job.Variant.SourceUrl.Host);
    ...
    private static void EnsureDownloadLooksComplete(DownloadJob job, long actualLength)
    {
        ...
        if (job.TotalBytes is long expected && actualLength != expected)
        {
            if ((softMedia && actualLength >= 1024) ||
                (softSignedProgressive && IsSoftSignedProgressiveAcceptable(actualLength)))
            {
        ...
        var minBytes = softSignedProgressive
            ? 256L * 1024
    ...
    private static bool IsSoftSignedProgressiveHost(Uri url)
    {
        var host = url.Host;
        return host.Contains("webapp-prime", StringComparison.OrdinalIgnoreCase) ||
               ...
               host.Contains("douyinvod", StringComparison.OrdinalIgnoreCase) ||
               host.Contains("zjcdn", StringComparison.OrdinalIgnoreCase);
    }

    private static bool IsSoftSignedProgressiveAcceptable(long received) =>
        received >= 256L * 1024;
