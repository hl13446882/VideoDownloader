...
public sealed partial class LocalizationService : ObservableObject
{
    ...
    private static readonly Dictionary<string, (string Zh, string En)> Table = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["auto.delayPrompt"] = ("设置自动限制（先到者生效；0 表示该项不限制）",
            "Set auto limits (first reached wins; 0 = no limit for that item)"),
        ["auto.delayMinutesLabel"] = ("时长（分钟，0 = 不限）", "Duration (minutes, 0 = unlimited)"),
        ["auto.delayCountLabel"] = ("视频数量（0 = 不限）", "Video count (0 = unlimited)"),
        ["auto.delayHint"] = ("达到时长或成功入队数量后结束自动（不再切换/入队）；进行中的下载会继续。两项都为 0 则仅手动停止。",
            "Stops auto when duration or successfully enqueued count is reached. In-progress downloads continue. Both 0 means manual stop only."),
        ["auto.delayInvalid"] = ("请输入非负整数。", "Enter a non-negative integer."),
        ["auto.ok"] = ("开始", "Start"),
        ["auto.cancel"] = ("取消", "Cancel"),
        ["status.autoRunning"] = ("自动模式中…剩余约 {0} 分 · 已入队 {1}", "Auto… ~{0} min left · enqueued {1}"),
        ["status.autoRunningCount"] = ("自动模式中…已入队 {0}/{1}", "Auto… enqueued {0}/{1}"),
        ["status.autoRunningUnlimited"] = ("自动模式中…已入队 {0}", "Auto… enqueued {0}"),
        ["status.autoWaitingSlot"] = ("自动模式：等待下载空位…剩余约 {0} 分 · 已入队 {1}", "Auto: waiting slot… ~{0} min left · enqueued {1}"),
        ["status.autoWaitingSlotCount"] = ("自动模式：等待下载空位…已入队 {0}/{1}", "Auto: waiting slot… enqueued {0}/{1}"),
        ["status.autoStoppedTimeout"] = ("自动模式已结束：到达执行时长", "Auto mode ended: duration reached"),
        ["status.autoStoppedCount"] = ("自动模式已结束：到达视频数量", "Auto mode ended: video count reached"),
        ...
        ["status.autoStarted"] = ("已进入自动模式（{0}）", "Auto mode on ({0})"),
        ["status.autoStartedLimits"] = ("{0} 分钟 / {1} 条", "{0} min / {1} videos"),
        ["status.autoStartedTimeOnly"] = ("{0} 分钟", "{0} min"),
        ["status.autoStartedCountOnly"] = ("{0} 条", "{0} videos"),
        ["status.autoStartedUnlimited"] = ("不限", "unlimited"),
        ...
        ["settings.subtitle.enabledHint"] = ("开启后：本地播放页识别字幕；未打开播放页时后台自动识别未完成的本地视频。网页在线视频不会启动识别。",
            "When on: recognize on the local play page; when the player is closed, unfinished library videos are recognized in the background. Online pages never start recognition."),
        ...
        ["update.launcherMissing"] = ("找不到启动器，无法自动升级。请手动重启。",
            "Launcher not found; cannot auto-update. Please restart manually."),
        ["update.launcherStartFailed"] = ("无法启动升级程序：{0}", "Cannot start the updater: {0}"),
        ...
        ["status.importBusy"] = ("正在导入本地文件夹，请稍候…", "A folder import is already running…"),
        ["status.importRunning"] = ("正在导入本地媒体 {0}/{1}…", "Importing local media {0}/{1}…"),
        ["status.importDone"] = ("导入完成：成功 {0}，失败 {1}（文件夹：{2}）",
            "Import finished: {0} ok, {1} failed (folder: {2})"),
        ["status.importFailed"] = ("导入失败：{0}", "Import failed: {0}"),
        ["import.pickFolderTitle"] = ("选择要导入的文件夹", "Choose a folder to import"),
        ["import.nameTitle"] = ("导入名称", "Import name"),
        ["import.namePrompt"] = ("保存文件夹名称（将创建在下载目录下）",
            "Save folder name (created under the download directory)"),
        ["import.defaultFolderName"] = ("导入", "Import"),
        ["import.successTitle"] = ("导入完成", "Import finished"),
        ["import.successBody"] = ("成功导入 {0} 个文件。\n跳过（不可播放或空文件）{1} 个，失败 {2} 个。\n保存文件夹：{3}",
            "Imported {0} file(s).\nSkipped (unplayable or empty): {1}. Failed: {2}.\nFolder: {3}"),
