...
public sealed class BilibiliYtDlpExtractor : IExternalSiteResolver
{
    private readonly AppOptions _options;
    private readonly ILogger<BilibiliYtDlpExtractor> _logger;
    private bool? _available;

    public BilibiliYtDlpExtractor(IOptions<AppOptions> options, ILogger<BilibiliYtDlpExtractor> logger)
    {
    ...
    private static bool SupportsHost(Uri pageUrl) => pageUrl.Host.Contains("bilibili", StringComparison.OrdinalIgnoreCase) || pageUrl.Host.Contains("b23.tv", StringComparison.OrdinalIgnoreCase);

    public bool SupportsSite(string siteId) => string.Equals(siteId, SiteIds.Bilibili, StringComparison.Ordinal);
        ...
        CancellationToken ct)
    {
        ...
        {
            LastError = "BilibiliYtDlpExtractor refuses foreign hosts";
            return [];
        }

        var siteId = SiteIds.Bilibili;
    ...
    {
        // Bilibili-only: strip fragment; leave BV/aid path intact.
